...
def parse_grammar(file, sentence):
    ...
    for line in c:
        ...
        if len(split_line) > 0:
            ...
            if len(check) == 1:
                if check[0] not in rules: #rule is more than one letter long and is all lowercase/numbers
                    if check[0] in split_sentence:
...
def main(argv):
    ...
    for line in f:
        if len(line.split()) > 0:
            sys.stderr.write("Starting sentence " + str(counter) + "\n")
            print(line)
            rules = parse_grammar(grammar, line)
            num = 0
